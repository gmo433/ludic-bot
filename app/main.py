...
# --- КЛЮЧЕВЫЕ СЛОВА ДЛЯ АКТИВАЦИИ В ГРУППЕ ---
FOOTBALL_KEYWORDS = [
    "футбол", "матч", "матчи", "футбольный", "бутсы", "гол", "голы", "пенальти", 
    "угловой", "аут", "офсайд", "судья", "вратарь", "нападающий", "защитник",
    "полузащитник", "тренер", "стадион", "болельщик", "чемпионат", "лига",
    "премьер-лига", "ла лига", "серия а", "бундеслига", "лига 1", "рпл",
    "лига чемпионов", "лига европы", "европа", "сборная", "клуб", "трансфер",
    "ставка", "ставки", "тотал", "фора", "победа", "ничья", "поражение",
    "счет", "результат", "таблица", "турнир", "очки", "голевая", "ассистент",
    "бомбардир", "дисциплина", "желтая", "красная", "карточка", "пенальти",
    "свободный", "штрафной", "удар", "голкипер", "капитан", "свисток",
    "дополнительное", "овертайм", "серия пенальти", "чемпион", "кубок",
    "суперкубок", "дерби", "классико", "ворота", "сетка", "штанга", "перекладина",
    "бутса", "мяч", "форма", "галоши", "щетки", "футболка", "трусы", "гетры",
    "перчатки", "вратарские", "шорты", "спортивный", "костюм", "разминка",
    "разминочный", "манеж", "поле", "газон", "искусственный", "естественный",
    "травяной", "крытый", "открытый", "стадион", "арена", "трибуна", "сектор",
    "вип-ложа", "пресс-центр", "раздевалка", "душ", "скамейка", "запасные",
    "замена", "резерв", "основной", "состав", "стартовый", "протокол",
    "соперник", "оппонент", "противник", "соперничество", "конкуренция",
    "конкурент", "соревнование", "турнир", "чемпионат", "первенство", "розыгрыш",
    "этап", "раунд", "круг", "тур", "игра", "встреча", "поединок", "противостояние",
    "битва", "баталия", "схватка", "победа", "успех", "триумф", "выигрыш",
    "поражение", "неудача", "проигрыш", "ничья", "ничейный", "ничейный результат",
    "разгром", "катастрофа", "трагедия", "сенсация", "неожиданность", "сюрприз",
    "фаворит", "аутсайдер", "темная лошадка", "лидер", "аутсайдер", "середняк",
    "топ", "дно", "зона", "вылет", "понижение", "повышение", "плей-офф",
    "стыковые", "полуфинал", "финал", "четвертьфинал", "осьмушка", "1/8",
    "1/4", "1/2", "бомбардир", "снайпер", "голеадор", "ассистент", "распасовщик",
    "диспетчер", "плеймейкер", "организатор", "капитан", "лидер", "ветеран",
    "молодой", "перспективный", "талант", "дарование", "звезда", "суперзвезда",
    "легенда", "идол", "кумир", "бог", "король", "королева", "принц", "принцесса",
    "наследник", "преемник", "новая", "старая", "гвардия", "смена", "поколение",
    "эра", "эпоха", "время", "период", "расцвет", "закат", "рассвет", "зенит",
    "пик", "вершина", "дно", "провал", "кризис", "перестройка", "реконструкция",
    "ребрендинг", "перезагрузка", "революция", "эволюция", "прогресс", "регресс",
    "развитие", "падение", "рост", "упадок", "расцвет", "стагнация", "застой",
    "прорыв", "рывок", "скачок", "рывок", "бросок", "прыжок", "взлет", "падение",
    "взлеты", "падения", "успехи", "неудачи", "достижения", "провалы", "рекорды",
    "достижения", "победы", "поражения", "ничьи", "очки", "голы", "пропущенные",
    "разница", "мячей", "забитые", "голы", "сухие", "матчи", "сухари", "вратарь",
    "отразил", "сейв", "сейвы", "момент", "голевой", "момент", "шанс", "возможность",
    "реализация", "голевая", "очередь", "очередность", "пенальти", "буллит",
    "один", "на", "один", "два", "на", "два", "три", "на", "три", "четыре",
    "на", "четыре", "пять", "на", "пять", "шесть", "на", "шесть", "семь",
    "на", "семь", "восемь", "на", "восемь", "девять", "на", "девять", "десять",
    "на", "десять", "ноль", "на", "ноль", "сухой", "матч", "ничья", "ничейный",
    "результат", "победа", "поражение", "разгром", "катастрофа", "трагедия",
    "сенсация", "неожиданность", "сюрприз", "фаворит", "аутсайдер", "темная",
    "лошадка", "лидер", "аутсайдер", "середняк", "топ", "дно", "зона", "вылет",
    "понижение", "повышение", "плей-офф", "стыковые", "полуфинал", "финал",
    "четвертьфинал", "осьмушка", "1/8", "1/4", "1/2", "бомбардир", "снайпер",
    "голеадор", "ассистент", "распасовщик", "диспетчер", "плеймейкер", "организатор",
    "капитан", "лидер", "ветеран", "молодой", "перспективный", "талант", "дарование",
    "звезда", "суперзвезда", "легенда", "идол", "кумир", "бог", "король", "королева",
    "принц", "принцесса", "наследник", "преемник", "новая", "старая", "гвардия",
    "смена", "поколение", "эра", "эпоха", "время", "период", "расцвет", "закат",
    "рассвет", "зенит", "пик", "вершина", "дно", "провал", "кризис", "перестройка",
    "реконструкция", "ребрендинг", "перезагрузка", "революция", "эволюция", "прогресс",
    "регресс", "развитие", "падение", "рост", "упадок", "расцвет", "стагнация", "застой",
    "прорыв", "рывок", "скачок", "рывок", "бросок", "прыжок", "взлет", "падение",
    "взлеты", "падения", "успехи", "неудачи", "достижения", "провалы", "рекорды",
    "достижения", "победы", "поражения", "ничьи", "очки", "голы", "пропущенные",
    "разница", "мячей", "забитые", "голы", "сухие", "матчи", "сухари", "вратарь",
...
async def on_bot_added_to_group(event: ChatMemberUpdated):
    ...
    if event.new_chat_member.user.id == (await bot.get_me()).id:
        ...
        kb.button(text="⚽ Открыть меню футбола", callback_data="group_menu")
        kb.adjust(1)
        
        await bot.send_message(
            event.chat.id,
            "⚽ *Футбольный бот добавлен в группу!* 🎉\n\n"
            "Я буду помогать с футбольной статистикой, матчами и ставками!\n\n"
            "💡 *Просто нажмите кнопку ниже чтобы начать:*",
            reply_markup=kb.as_markup(),
            parse_mode="Markdown"
        )

# --- ОБРАБОТЧИК КЛЮЧЕВЫХ СЛОВ В ГРУППЕ ---
@dp.message(Text(text=FOOTBALL_KEYWORDS, ignore_case=True))
async def handle_football_keywords(message: types.Message):
    """Обработчик футбольных ключевых слов в группе"""
    if message.chat.type in ["group", "supergroup"]:
        # Проверяем, что сообщение не от самого бота
        if message.from_user.id != (await bot.get_me()).id:
            # Создаем клавиатуру с кнопкой меню
            kb = InlineKeyboardBuilder()
            kb.button(text="⚽ Показать меню футбола", callback_data="group_menu")
            kb.adjust(1)
            
            await message.reply(
                "⚽ *Вижу, вы говорите о футболе!* 🎯\n\n"
                "Хотите посмотреть актуальную информацию?",
                reply_markup=kb.as_markup(),
                parse_mode="Markdown"
            )

# --- ОБРАБОТЧИК КНОПКИ МЕНЮ ДЛЯ ГРУПП ---
@dp.callback_query(lambda c: c.data == "group_menu")
async def process_group_menu(callback: types.CallbackQuery):
    """Обработчик кнопки меню в группе"""
    await callback.answer("⚽ Открываю меню...")
    
    kb = InlineKeyboardBuilder()
    
    # Основные функции
    kb.button(text="📅 Матчи", callback_data="get_matches")
    kb.button(text="📡 LIVE", callback_data="get_live")
    kb.button(text="🎲 Ставка", callback_data="random_bet")
    
    # Статистика
    kb.button(text="🏆 Лиги", callback_data="select_league") 
    kb.button(text="📊 Таблицы", callback_data="tables_menu")
...
async def main():
    ...
    # Запускаем поллинг
    log.info("🚀 Запуск бота с поддержкой групп...")
...
if __name__ == "__main__":
    log.info("🚀 Запуск бота с улучшенным визуалом и поддержкой групп")
