        ...
        # --- Backend endpoint to fetch matches (server side; uses API_SPORT_KEY) ---
        @app.get("/api/matches")
        def api_matches():
            try:
                now = datetime.utcnow()
                ...
                # Endpoint path adapted to API-Sport; if needed, adjust to exact path
        ...
        async def cmd_start(message: types.Message):
            ...
            # web_app URL should point to public URL of this service once deployed.
            # Telegram requires an HTTPS URL accessible from internet.
            kb.button(
...
Нажми кнопку ниже, чтобы открыть мини‑приложение с ближайшими матчами:",
        ...
        def run_api():
            # uvicorn will serve the FastAPI app on 0.0.0.0:8080
            uvicorn.run(app, host="0.0.0.0", port=8080)

        if __name__ == "__main__":
            # Start API and bot in separate threads
